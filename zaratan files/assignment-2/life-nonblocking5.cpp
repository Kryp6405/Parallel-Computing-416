...
void write_output(int *life, int local_rows, int Y_limit, int cols, int offset, const char *input_name, int num_of_generations, int rank, int size) {
    ...
    for (int i = 1; i <= local_rows; i++) {
        for (int j = 1; j <= Y_limit; j++) {
            ...
            if (life[idx] == 1) {
                ...
                local_live_cells[local_count++] = offset + i - 1; // x coordinate
                local_live_cells[local_count++] = j - 1;          // y coordinate
            }
        }
    }
    ...
    int local_count_ints = local_count; // Number of integers to send
    ...
    if (rank == 0) {
        ...
        strncpy(output_file_name, input_name, strlen(input_name) - 4); // Remove ".csv"
...
int main(int argc, char *argv[]) {
    ...
    // Free live_cells as it's no longer needed
    if (live_cells != NULL) {
        free(live_cells);
    }
